...
class PlaybookGenerator:
    """
    Generate Ansible playbook from profile resolution.

    This class handles reading an existing playbook, preserving its
    pre_tasks and vars_prompt sections, and writing a complete playbook
    with generated roles based on profile resolution.
    """

    def __init__(self, profile: str, profiles_dir: str = _DEFAULT_PROFILES_DIR):
        """
        Initialize the playbook generator.

        Args:
            profile: Profile name to generate roles for
    ...
    def write_playbook(self, output_path: str) -> None:
        """
        Write a complete playbook with generated roles.

        Reads the existing playbook at output_path to extract pre_tasks
        and vars_prompt, then writes a new playbook with generated roles.

        Args:
            output_path: Path to write the playbook (will overwrite existing)
        """
        # Read existing playbook to extract pre_tasks and vars_prompt
        playbook_path = Path(output_path)
        ...
        # Extract pre_tasks and vars_prompt from existing playbook
        # Handle list of plays format
        if isinstance(existing_data, list):
            plays = existing_data
            pre_tasks = plays[0].get("pre_tasks", []) if plays else []
            vars_prompt = plays[0].get("vars_prompt", []) if plays else []
        else:
            pre_tasks = existing_data.get("pre_tasks", [])
            vars_prompt = existing_data.get("vars_prompt", [])

        # Resolve role manifest to generate roles
        manifest = resolve_role_manifest(
            profile=self.profile,
            os_family="Archlinux",
            host_vars={},
            profiles_dir=self.profiles_dir,
            preserve_config_check=False,
        )

        # Write playbook with header comment
        with open(playbook_path, "w") as f:
            ...
            # Write pre_tasks (preserved verbatim)
            if pre_tasks:
                f.write("  pre_tasks:\n")
                self._write_yaml_block(f, pre_tasks, indent=4)
                f.write("\n")

            # Write roles with section comments
            f.write("  roles:\n")
            self._write_roles_with_sections(f, manifest.roles)
            f.write("\n")

            # Write vars_prompt (preserved verbatim)
            if vars_prompt:
                f.write("  vars_prompt:\n")
                self._write_yaml_block(f, vars_prompt, indent=4)
                f.write("\n")

    def _write_roles_with_sections(self, f, roles: Tuple[RoleCondition, ...]) -> None:
        """
        Write roles with section comments.

        Args:
            f: File object to write to
            roles: Tuple of RoleCondition entries
        """
        current_section = None

        for role_cond in roles:
            # Determine section for this role
            section = self._get_section_for_role(role_cond.role)

            # Add section comment if section changed
            ...
            # Write role entry
            tags_str = str(list(role_cond.tags))
            if role_cond.condition:
                # Check if condition is simple enough for flow style
                if "\n" not in role_cond.condition and len(role_cond.condition) < 60:
                    f.write(f"  - {{ role: {role_cond.role}, tags: {tags_str}, when: {role_cond.condition} }}\n")
                else:
                    # Use block style for complex conditions
                    f.write(f"  - role: {role_cond.role}\n")
                    f.write(f"    tags: {tags_str}\n")
                    f.write(f"    when: \"{role_cond.condition}\"\n")
            else:
                f.write(f"  - {{ role: {role_cond.role}, tags: {tags_str} }}\n")

    def _write_yaml_block(self, f, data: Any, indent: int) -> None:
        """
        Write a YAML block with proper indentation.

        Args:
            f: File object to write to
            data: Data to write (will be converted to YAML)
    ...
    def _get_section_for_role(self, role_name: str) -> str:
        """
        Get the section name for a role.

        Args:
            role_name: Name of the role

        Returns:
            Section name or "Other" if not found in any section
        """
